...
def test_declared_width_ignores_a_same_named_table_in_an_attached_catalog(tmp_path):
    ...
    catalog. DESCRIBE resolves the bare name through the same catalog/schema search path a real
    query against the table would use, so an attached catalog's same-named table cannot decide
    the answer.
    """

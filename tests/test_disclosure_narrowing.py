...
def test_a_mask_the_query_never_REFERENCED_is_not_reported_though_the_rewrite_applies_it():
    ...
    This is the UNDER case, and it is the only one left. The OVER case -- a same-named column on
    another table setting the flag here -- was real when this test was written and is fixed:
    ownership resolves through `column_tables`, and `tests/test_mask_attribution.py` guards both
    that and the unresolved cases that must still over-mask.

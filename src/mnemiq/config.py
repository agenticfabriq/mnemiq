...
class Settings(BaseSettings):
    ...
    verity_records_url: str | None = Field(default=None, description="Verity GET /api/semantic/records/open endpoint (governed certified records). NOT the sibling /api/semantic/records, and DO NOT reach for a smaller page size to make that one fit: it accepts no `since`, so a page size it tolerates makes every pull a full dump merged as though it were a delta, silently, while this pull's default 500 makes it refuse outright. Loud is the better failure. Verity's published OpenAPI is where that endpoint's missing `since` and its own limit ceiling are declared")

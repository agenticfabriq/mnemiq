"""Spider 2.0-lite helpers for the VENDOR benchmark path (Genie, Cortex, ai_query).

Deliberately separate from `mnemiq.eval.spider2`, which is the ENGINE's implementation and is
owned by another workstream. The two diverged underneath a live sweep: the engine's loader now
takes a dataset DIRECTORY and builds `<dir>/repo/spider2-lite/...` plus `<dir>/databases/` inside
it, a layout the engine path materialises and which the vendor path never had. Calling it with
the instance file the vendor runners have always passed crashed on

    .../spider2-lite.jsonl/repo/spider2-lite/spider2-lite.jsonl

and its grading helpers (compare_tables, gold_paths, load_eval_meta) no longer exist there at all,
so vendor grading would have failed too.

Keeping a vendor copy also pins two things that must not drift if new runs are to be comparable
with the ones already published: the exact wording used to append a reference document to a
question, and `compare_tables` as a faithful port of Spider 2.0's own `compare_pandas_table`.
"""

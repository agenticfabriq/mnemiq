...
def all_genie_spaces(workspace) -> list:
    """Every Genie space in the workspace, following pagination to the end.

    `workspace.genie.list_spaces()` returns ONE page -- 20 spaces -- plus a next_page_token.
    Reading `.spaces` and stopping looks like a complete listing and is not. That mistake is
    expensive in both directions: it reported 41 of 61 benchmark spaces as deleted when all of
    them existed, and then it silently created 41 duplicates while "recreating" them.
    """
    spaces: list = []
    token = None
    while True:
        page = workspace.genie.list_spaces(page_token=token) if token else workspace.genie.list_spaces()
        spaces.extend(page.spaces or [])

...
from mnemiq.eval.spider2 import compare_tables, gold_paths, load_eval_meta
from mnemiq.eval.warehouse import load_results
...
def run_sql(cur, database: str, schema: str, sql: str) -> list[list]:
    ...
    try:
        for index, (case_id, result) in enumerate(sorted(results.items()), 1):
            ...
            if not golds:
                ...
                try:
                    predicted = run_sql(cur, args.database, schema, result.sql)

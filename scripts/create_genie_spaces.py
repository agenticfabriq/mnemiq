...
def main() -> int:
    ...
    live = all_genie_spaces(workspace)
    existing = {s.title: s.space_id for s in live}
    live_ids = {s.space_id for s in live}
    ...
    # A mapped id whose space has been deleted is worse than no mapping at all: the entry looks
    # complete, so this script skips it as "already mapped" and the runner then asks a space
    # that does not exist. Drop dead entries here so they are rebuilt.
    dead = sorted(db for db, sid in mapping.items() if sid not in live_ids)
    if dead:
        print(f"{len(dead)} mapped spaces no longer exist and will be recreated: "
              f"{', '.join(dead)}")
        for db in dead:
            del mapping[db]

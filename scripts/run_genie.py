...
def main() -> int:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="run even though some databases have no live Genie space. Off by default: a "
        "partial sweep that reports itself as complete is worse than no sweep.",
    )
    p.add_argument(
        "--print-spaces",
        action="store_true",
    ...
    if args.print_spaces:
        for space in all_genie_spaces(workspace):
    ...
    # Every database the question set needs must resolve to a space that EXISTS RIGHT NOW.
    #
    # Checking the map alone is not enough, and that distinction is the whole point: a run once
    # had all 61 databases present in the JSON while 41 of the space ids behind them had been
    # deleted from the workspace. The old behaviour printed one line and carried on, so the
    # sweep ran to completion and reported BIRD 0 of 500 asked -- hours of warehouse time, no
    # error, and a "finished" results file that looked like data. A benchmark harness that
    # answers fewer questions than it was asked to must stop, not narrate.
    needed = sorted({c.db_id for c in cases})
    live_ids = {s.space_id for s in all_genie_spaces(workspace)}
    unmapped = [d for d in needed if d not in spaces]
    dead = [d for d in needed if d in spaces and spaces[d] not in live_ids]

    if unmapped or dead:
        for label, group in (("no space in the map", unmapped), ("space id no longer exists", dead)):
            if group:
                print(f"{len(group)} databases -- {label}:", file=sys.stderr)
                for db_id in group:
                    detail = f"  {db_id}" + (f"  ({spaces[db_id]})" if db_id in spaces else "")
                    print(detail, file=sys.stderr)
        skipped = len([c for c in cases if c.db_id in set(unmapped) | set(dead)])
        if not args.allow_missing_spaces:
            print(
                f"\nrefusing to run: {skipped} of {len(cases)} questions have no live Genie "
                f"space. Recreate them (scripts/create_genie_spaces.py) or pass "
                f"--allow-missing-spaces to run the remainder deliberately.",
                file=sys.stderr,
            )
            return 2
        print(f"\n--allow-missing-spaces: continuing without {skipped} of {len(cases)} questions")

    live_dbs = {d for d in needed if d in spaces and spaces[d] in live_ids}
    remaining = [c for c in cases if c.id not in done and c.db_id in live_dbs]
